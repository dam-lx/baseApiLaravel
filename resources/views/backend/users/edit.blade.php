							...
							<div class="form-group">
								<div class="input-group toggle lg">
									<label  class="toggle-flip">
										<input type="checkbox" id="active" @if($user->is_active == \App\Core\Common\UserConst::active) {{"checked"}} @endif>
										<span class="flip-indecator" data-toggle-on="{{trans('label.on')}}" data-toggle-off="{{trans('label.off')}}" ></span>
									</label>
								</div>
							</div>
						</div>
						<div class="tile-footer">
							<div class="row">
								<div class="col-md-12 col-md-offset-3 t-r">
									<button type="button" class="btn btn-success edit_account">{{trans('common.save')}}</button>
